...
export type Output221222 = {
    ...
    mostTalkativeInRoom: {
        [chatRoomId: string]: string; // key is chatRoomId, value is senderId
    };
    mostBusyPerson: string; // senderId
};
...
function theMostTalkative(){
    const theMostTalkativePerson = {};
    for(let i = 0; i < input221222.length; i++){
      if(input221222[i]['senderId'] in theMostTalkativePerson){
        theMostTalkativePerson[input221222[i]['senderId']] +=1;
      }else{
        theMostTalkativePerson[input221222[i]['senderId']] =1;
      }
    }
    // const arr = Object.values(theMostTalkativePerson);
    // const findingTalkative = Math.max(...arr);
  
    let maxValue = 0;
    let maxSenderId = '';
   
  
    for(let person in theMostTalkativePerson){
      if(theMostTalkativePerson[person] > maxValue){
        maxValue = theMostTalkativePerson[person];
        maxSenderId = person;
      }
    }
    return maxSenderId;
...
function mostTalkativeInRoom(){
    const eachRoom = {};
    for(let i=0; i< input221222.length;i++){
      if(eachRoom[input221222[i]['charRoomId']]) {
        if(eachRoom[input221222[i]['chatRoomId']][input221222[i]['senderId']]){
          eachRoom[input221222[i]['chatRoomId'][input221222[i]['senderId']]] +=1;
        } else{
          eachRoom[input221222[i]['chatRoomId']][input221222[i]['senderId']] =1
      }
    }else{
      eachRoom[input221222[i]['chatRoomId']] = {};
      eachRoom[input221222[i]['chatRoomId']][input221222[i]['senderId']] = 1;
    }
  ...
  console.log(mostTalkativeInRoom())
  //{챗룸아이디 : {샌더아이디 : 카운트}}
     
  

// Find who is participating in most chat room
function participatingInMostChatRoom(){
    const senders = {};
    \
    for(let i = 0; i < input221222.length; i++){
      senders[input221222[i]['senderId']] = null;
      if(input221222[i]['senderId'] && input221222[i]['charRoomId']){
        sendersCount[input221222[i]['charRoomId']] += 1
      }
  ...
  console.log(participatingInMostChatRoom())
